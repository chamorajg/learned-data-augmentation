...
class VanillaVAE(BaseVAE):
    ...
    def forward(self, input: Tensor, **kwargs) -> List[Tensor]:
        ...
        mu2, log_var2 = self.encode2(output)
        z2 = self.reparameterize(mu2, log_var2)
        return  [self.decode2(z2), output, mu2, log_var2], [self.decode(z), input, mu, log_var]
    ...
    def loss_function(self,
        ...
        recons_loss = F.binary_cross_entropy(recons, undistorted_input, reduction='sum')
        recons_loss2 = F.binary_cross_entropy(recons, input, reduction='sum')

        kld_loss = torch.mean(-0.5 * torch.sum(1 + log_var - mu ** 2 - log_var.exp(), dim = 1), dim = 0)
        kld_loss2 = torch.mean(-0.5 * torch.sum(1 + log_var2 - mu2 ** 2 - log_var.exp(), dim = 1), dim = 0)

        loss = recons_loss + kld_weight * kld_loss + recons_loss2 + kld_weight * kld_loss2
