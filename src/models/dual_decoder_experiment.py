...
class DualDecoderVAEXperiment(pl.LightningModule):

    def __init__(self,
                 vae_model: BaseVAE,
                 params: dict) -> None:
        super(DualDecoderVAEXperiment, self).__init__()
    ...
    def sample_images(self):
        ...
        if len(test_input) > 1:
            vutils.save_image(test_input[1].data,
                            f"{self.logger.save_dir}{self.logger.name}/version_{self.logger.version}/"
                            f"input_augment_{self.logger.name}_{self.current_epoch}.png",
                            normalize=True,
                            nrow=8)
        test_input = test_input[0].to(self.curr_device)
        test_label = test_label.to(self.curr_device)
        recons = self.model.generate(test_input, labels = test_label)
        vutils.save_image(test_input.data,
                          f"{self.logger.save_dir}{self.logger.name}/version_{self.logger.version}/"
                          f"input_undistorted_{self.logger.name}_{self.current_epoch}.png",
                          normalize=True,
                          nrow=8)
        vutils.save_image(recons[1].data,
                          f"{self.logger.save_dir}{self.logger.name}/version_{self.logger.version}/"
                          f"recons_augmented_{self.logger.name}_{self.current_epoch}.png",
                          normalize=True,
                          nrow=8)
        vutils.save_image(recons[0].data,
                          f"{self.logger.save_dir}{self.logger.name}/version_{self.logger.version}/"
                          f"recons_undistorted_{self.logger.name}_input_{self.current_epoch}.png",
                          normalize=True,
                          nrow=8)
